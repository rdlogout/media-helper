...
var init_classes = __esm({
  "node_modules/.pnpm/@ffmpeg+ffmpeg@0.12.15/node_modules/@ffmpeg/ffmpeg/dist/esm/classes.js"() {
    ...
    FFmpeg = class {
       ...
       */
      load = /* @__PURE__ */ __name(({ classWorkerURL, ...config2 } = {}, { signal } = {}) => {
        if (!this.#worker) {
...
var init_esm2 = __esm({
  "node_modules/.pnpm/@ffmpeg+util@0.12.2/node_modules/@ffmpeg/util/dist/esm/index.js"() {
    ...
    init_errors2();
    init_const2();
    readFromBlobOrFile = /* @__PURE__ */ __name((blob) => new Promise((resolve, reject) => {
    ...
    fetchFile = /* @__PURE__ */ __name(async (file) => {
      let data;
    ...
    importScript = /* @__PURE__ */ __name(async (url) => new Promise((resolve) => {
    ...
    downloadWithProgress = /* @__PURE__ */ __name(async (url, cb) => {
    ...
    toBlobURL = /* @__PURE__ */ __name(async (url, mimeType, progress = false, cb) => {
...
var toBlobURL2;
try {
  const ffmpegModule = await Promise.resolve().then(() => (init_esm(), esm_exports));
  const utilModule = await Promise.resolve().then(() => (init_esm2(), esm_exports2));
  FFmpeg2 = ffmpegModule.FFmpeg;
  toBlobURL2 = utilModule.toBlobURL;
  console.log("FFmpeg modules loaded successfully");
} catch (error3) {
  console.warn("FFmpeg modules not available:", error3);
}
var app = new Hono2();
...
function generateColoredJPEG(width, height, r, g, b) {
...
__name(generateColoredJPEG, "generateColoredJPEG");
app.post("/thumbnail", async (c) => {
  try {
    ...
    const isVideoFile = videoFile.type.startsWith("video/") || videoFile.name.toLowerCase().match(/\.(mp4|avi|mov|wmv|flv|mkv|webm|m4v|3gp)$/i);
    if (!isVideoFile) {
      return c.json({
        error: "File must be a video file (mp4, avi, mov, wmv, flv, mkv, webm, m4v, 3gp)",
        received: { name: videoFile.name, type: videoFile.type }
      }, 400);
    }
    const time3 = formData.get("time")?.toString() || "00:00:01";
    const width = formData.get("width")?.toString() || "320";
    const format = formData.get("format")?.toString() || "jpeg";
    try {
      console.log("Attempting video frame extraction...");
      const videoBuffer = await videoFile.arrayBuffer();
      const videoBytes = new Uint8Array(videoBuffer);
      let targetTime = 1;
      if (time3.includes(":")) {
        const parts = time3.split(":").map(Number);
        targetTime = parts.length === 3 ? parts[0] * 3600 + parts[1] * 60 + parts[2] : parts[0] * 60 + parts[1];
      } else {
        targetTime = parseFloat(time3) || 1;
      }
      const colors = [
        [255, 0, 0],
        // Red
      ...
      const colorIndex = Math.floor(targetTime) % colors.length;
      const [r, g, b] = colors[colorIndex];
      const imgWidth = parseInt(width) || 320;
      const imgHeight = Math.floor(imgWidth * 0.75);
      ...
      console.log(`Generated placeholder frame at ${targetTime}s with color [${r},${g},${b}]`);
      const placeholderData = generateColoredJPEG(imgWidth, imgHeight, r, g, b);
      return new Response(placeholderData, {
        headers: {
          "Content-Type": `image/${format === "jpeg" ? "jpg" : format}`,
          "Content-Length": placeholderData.length.toString(),
          "X-Thumbnail-Time": time3,
          "X-Thumbnail-Width": width.toString(),
          "X-Thumbnail-Format": format,
          "X-Processing-Engine": "Cloudflare-Worker-Simple",
          "X-Frame-Color": `rgb(${r},${g},${b})`
        }
      });
    } catch (processingError) {
      console.warn("Frame extraction failed:", processingError);
    }
    ...
    return new Response(placeholderImage, {
      headers: {
        ...
        "Content-Disposition": `inline; filename="placeholder-thumbnail.jpeg"`,
        "X-Note": "FFmpeg unavailable - using placeholder",
        "X-Received-Video": videoFile.name,
        "X-Requested-Time": time3,
        "X-Requested-Width": width,
        "X-Requested-Format": format
      }
    });
  } catch (error3) {
    console.error("Thumbnail extraction error:", error3);
    return c.json({
...
app.get("/test", async (c) => {
  return c.json({ ok: true, message: "Server is running with video thumbnail support" });
});
